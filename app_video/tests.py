...
def video():
    video = Video.objects.create(
        title="test", description="test", url="http://test.com"
    )
    return video
...
def test_create_video(api_client, category):
    ...
    video = api_client.post(
        "/api/videos/",
        data={"title": "test", "description": "test", "url": "http://teste.com"},
        format="json",
    )
...
def test_delete_video(api_client, category, video):
    video_deleted = api_client.delete(path=f"/api/videos/{video.id}/", format="json")
...
def test_search_video_title(api_client, video):
    esperado = status.HTTP_200_OK
    resultado = api_client.get(path=f"/api/videos/?search=test", format="json")
