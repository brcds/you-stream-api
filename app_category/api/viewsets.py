...
class CategoryViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"detail": "Video Incluido com Sucesso!"}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def update(self, request, *args, **kwargs):
        try:
            categoria = self.get_object()
            serializer = self.get_serializer(categoria, data=request.data, partial=False)
            ...
            return Response({'detail': 'Categoria não encontrado.'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            return Response({'detail': 'Categoria deletado com sucesso!.'}, status=status.HTTP_200_OK)
        except Http404:
            return Response({'detail': 'Categoria não encontrado.'}, status=status.HTTP_404_NOT_FOUND)
